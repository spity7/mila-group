import { IServiceDetails } from "@/constant/DesignAgency/service-details/service-details-data";
...
const ServiceDetailsSection:React.FC<ServiceDetailsProps> = ({type,data:serviceDetailsData}) => {
  return (
    <section className="service-details-area">
      <div className={`service-details-area-inner ${type === 'v2'?"section-spacing":"section-spacing-bottom"}`}>
        ...
        {serviceDetailsData?.video?.src && (
          <div className="video-box fade-anim" suppressHydrationWarning={true}>
          ...
          <div className="section-content">
            ...
            {serviceDetailsData?.content?.paragraphs?.length > 0 &&
              serviceDetailsData?.content.paragraphs.map((para, idx) => (
                <div key={idx} className="text-wrapper fade-anim" suppressHydrationWarning={true}>
